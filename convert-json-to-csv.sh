...
for ARGUMENT in "$@"
    ...
    docker run cyclonedx/cyclonedx-cli convert --input-file ./sboms/components/$IMAGE_NAME/$IMAGE_NAME-$IMAGE_TAG.sbom.json --output-file sboms/components/$IMAGE_NAME/$IMAGE_NAME-$IMAGE_TAG.sbom.csv

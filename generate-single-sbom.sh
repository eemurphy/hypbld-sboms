...
for ARGUMENT in "$@"
    ...
    if [ ! -d sboms/$FILE_NAME/ ]; then
        mkdir sboms/$FILE_NAME/
    fi
    mkdir sboms/$FILE_NAME/
    syft docker:$ORG/$IMAGE_NAME:$IMAGE_TAG -o cyclonedx-json=sboms/$FILE_NAME/$IMAGE_NAME-$IMAGE_TAG.sbom.json
    # syft docker:quay.io/$PIPELINE_ORG/$PIPELINE_IMAGE_NAME:$PIPELINE_IMAGE_TAG -o cyclonedx-json
    # cyclonedx-cli convert --input-file sboms/components/$IMAGE_NAME/$IMAGE_NAME-$IMAGE_TAG.sbom.json --output-file sboms/components/$IMAGE_NAME/$IMAGE_NAME-$IMAGE_TAG.sbom.csv
